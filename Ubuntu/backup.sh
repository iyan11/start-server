...
sudo chown -R postgres:www-data $BACKUP_DIR
sudo usermod -aG www-data postgres
sudo chmod 2775 $BACKUP_DIR

# Создание .env файла
ENV_FILE=".env"
cat <<EOL > $BACKUP_DIR/$ENV_FILE
...
BACKUP_SCRIPT="backup_pgsql.sh"
cat <<'EOS' > $BACKUP_DIR/$BACKUP_SCRIPT
#!/bin/bash
...
EOS
echo "Скрипт backup_pgsql.sh создан."

# Делаем скрипт исполняемым
sudo chmod +x $BACKUP_DIR/$BACKUP_SCRIPT

# Добавление задачи в crontab через sudo
CRON_JOB="$CRONTAB_TIME cd $BACKUP_DIR && /bin/bash $BACKUP_DIR/backup_pgsql.sh"
sudo bash -c "(crontab -u $CRON_USER -l 2>/dev/null; echo \"$CRON_JOB\") | crontab -u $CRON_USER -"
if [ $? -eq 0 ]; then
  echo "Задача добавлена в crontab пользователя $CRON_USER: $CRON_JOB"
else
